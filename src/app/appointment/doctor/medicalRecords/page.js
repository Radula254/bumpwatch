    ...
    <>
      ...
      <section className="min-h-screen flex flex-col items-center bg-gradient-to-r from-pink-100 via-purple-100 to-indigo-100 py-20">
        <div className="max-w-4xl w-full">
          ...
          <div className="mt-8">
            {users?.filter(user => user?.userInfo?.createdAppointment === true).length > 0 ? (
              users.filter(user => user?.userInfo?.createdAppointment === true).map((user) => (
                <div key={user._id} className="bg-white border border-gray-200 rounded-xl shadow-lg mb-6 p-6 flex items-center gap-6">
                  <div className="flex flex-col md:flex-row md:items-center w-full">
                    ...
                    <div className="mt-4 md:mt-0 md:ml-4">
                      <Link className="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition" href={`/appointment/doctor/medicalRecords/${user._id}`}>
                        Consult
                      </Link>
                    </div>
                    <div className="mt-4 md:mt-0 md:ml-4">
